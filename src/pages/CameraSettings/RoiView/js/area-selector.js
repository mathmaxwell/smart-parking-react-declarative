...
function getRotatePos(
	[newTop, newLeft, newWidth, newHeight],
	[currentTop, currentLeft, currentWidth, currentHeight],
	angle
) {
	if (!angle) {
...
const createRect = (
		...
		const dragHandler = ({ pageX, pageY }) => {
...

			{
				const { top, left, height, width } =
					area.parentElement.getBoundingClientRect()
				areaHeight = height
				areaWidth = width
			...
			const moveTopLeft = () =>
				getRotatePos(
					[y1, x1, WIDTH + dx, HEIGHT + dy],
					[TOP, LEFT, WIDTH, HEIGHT],
					ANGLE
				)
			const moveTopRight = () =>
				getRotatePos(
					[y1, LEFT, x1 - LEFT, HEIGHT + dy],
					[TOP, LEFT, WIDTH, HEIGHT],
					ANGLE
				)
			const moveBottomLeft = () =>
				getRotatePos(
					[TOP, x1, WIDTH + dx, y1 - TOP],
					[TOP, LEFT, WIDTH, HEIGHT],
					ANGLE
				)
			const moveBottomRight = () =>
				getRotatePos(
					[TOP, LEFT, x1 - LEFT, y1 - TOP],
					[TOP, LEFT, WIDTH, HEIGHT],
					ANGLE
				)
	...
	const resize = ([image, area, root]) => {
		...
		const [prevWidth, prevHeight, prevTop, prevLeft] = [
			WIDTH / KX,
			HEIGHT / KY,
			TOP / KY,
			LEFT / KX,
...
const createRoi = (
	...
	const createControl = (
		...
		const dragHandler = ({ pageX, pageY }) => {
			...
			x1 = max(
				leftAnchor
					? min(pageX - left, width - RIGHT)
					: min(-1 * (pageX - right), width - LEFT),
				0
			)
			y1 = max(
				topAnchor
					? min(pageY - top, height - BOTTOM)
					: min(-1 * (pageY - bottom), height - TOP),
				0
			)
			setHorizontal(x1)
			setVertical(y1)
	...
	const [area, rect, content, setTop, setLeft, setRight, setBottom] =
		createArea()
	...
	;[topLeft, topRight, bottomLeft, bottomRight].forEach(node =>
...
const waitForSize = (img, runOutsideAngular = RUN_OUTSIDE_ANGULAR) =>
	new Promise(res => {
		const check = () => {
		...
		const interval = runOutsideAngular(() =>
			setInterval(() => {
				if (check()) {
...
function AreaSelector(resize = resizeHandler) {
	...
	const resizePipeline = new (class {
		...
		exec = ([i, a, r]) =>
			this.composes.reduce((acm, cur) => ([i, a, r]) => {
				acm([i, a, r])
				cur([i, a, r])
			})([i, a, r])
	})()
	...
	IMAGE_SOURCE = self.hasAttribute('imageSrc')
		? self.getAttribute('imageSrc')
		: IMAGE_SOURCE
	...
	const setControls = async controls => {
		...
		for (const [type, ...args] of controls.sort(([a]) =>
			a === 'roi' ? -1 : 1
		)) {
	...
	RUN_OUTSIDE_ANGULAR(() =>
		setTimeout(() =>
			AREA_REFERENCE_CALLBACK(ID, createControlObject({ setControls }))
		)
	)
